...
public ArrayList<String> deck(int pokemonCount) {
    ArrayList<String> deck = new ArrayList<>();           //creating an arraylist that's called deck
    
    int energyCount = 60 - pokemonCount;                  //energy count will always be the remaining amount of cards left after subtracting from pokemon cards
...
public double oddsOfMulligan(int pokemonCount, int userInputNumbers){
    ...
    for(int i = 0; i < userInputNumbers; i++){                                   //loop that runs depending on what the userinput is
        ArrayList<String> shuffledDeck = new ArrayList<>(deck(pokemonCount));
